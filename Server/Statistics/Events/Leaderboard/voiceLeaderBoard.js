const { Collection, Colors, PermissionFlagsBits } = require('discord.js'); 
...
module.exports = async (client) => {
    ...
    const data = Database ? Database.Caches : {};
    ...
    async function checkingLeader() { 
        ...
        let messageUsers = data.sort((uye1, uye2) => {
            ...
            let sunucu = client.guilds.cache.get(sistem.SERVER.ID);
            let uyecik = sunucu.members.cache.get(m.userID)
            if(sunucu && uyecik && index == 0 && !onecik.includes(uyecik.id)) onecik.push(uyecik.id); 
            return `\` ${index + 1} \` <@${m.userID}>: \`${Number(uyeToplam)} mesaj\``;
        }).join('\n');
        
        let sunucu = client.guilds.cache.get(sistem.SERVER.ID);

        // Haftanın Birincisi Rol İşlemleri (V14 İzin Yapısı Entegre Edildi)
        if(ayarlar && ayarlar.haftaBirincileri && ayarlar.haftaninBirinciRolü && onecik && onecik.length > 0) {
     
            let eklenecek = sunucu.members.cache.filter(x => 
                !roller.kayıtsızRolleri.some(rol => x.roles.cache.has(rol)) && 
                !x.permissions.has(PermissionFlagsBits.ManageRoles) && 
                !x.permissions.has(PermissionFlagsBits.ManageChannels) && 
                !x.permissions.has(PermissionFlagsBits.ManageGuild)  && 
                !x.roles.cache.has(roller.underworldRolü) && 
                !x.roles.cache.has(roller.jailRolü) && 
                !x.roles.cache.has(roller.şüpheliRolü) && 
                !x.roles.cache.has(roller.yasakTagRolü) && 
                !ayarlar.staff.includes(x.id) && 
                !x.permissions.has(PermissionFlagsBits.Administrator) && 
                !roller.kurucuRolleri.some(rol => x.roles.cache.has(rol)) && 
                !roller.Yetkiler.some(rol => x.roles.cache.has(rol))  && 
                !x.roles.cache.has(ayarlar.haftaninBirinciRolü) && 
                onecik.includes(x.id))
...
async function yayınSıralaması(guild) {
    ...
    Data = Data.filter(x => guild.members.cache.has(x._id) && x.Streaming).sort((a, b) => {
        let uye2Toplam2 = 0;
        b.Streaming.map(x => { uye2Toplam2 += Number(x.End - x.Start) })
        let uye1Toplam2 = 0;
        a.Streaming.map(x => { uye1Toplam2 += Number(x.End - x.Start) })
        return uye2Toplam2 - uye1Toplam2 
    }).map((result, index) => {
        let uyeToplam2 = 0;
        result.Streaming.map(x => { uyeToplam2 += Number(x.End - x.Start) })
    ...
    let embed = new genEmbed().setAuthor({name: guild.name, iconURL: guild.iconURL({dynamic:true})}) // HATA DÜZELTİLDİ
...
async function davetSıralaması(guild) {
    ...
    Data = Data.filter(x => guild.members.cache.has(x.userID) && x.total).sort((a, b) => Number(b.total) - Number(a.total)).map((result, index) => {
...
async function taglıSıralaması(guild) {
    ...
    if(!kanal) return;
    let Data = await Users.find()
    Data = Data.filter(x => guild.members.cache.has(x._id) && x.Taggeds).sort((a, b) => Number(b.Taggeds.length) - Number(a.Taggeds.length)).map((result, index) => {
...
async function yetkiliSıralaması(guild) {
    ...
    if(!kanal) return;
    let Data = await Users.find()
    Data = Data.filter(x => guild.members.cache.has(x._id) && x.Staffs).sort((a, b) => Number(b.Staffs.length) - Number(a.Staffs.length)).map((result, index) => {
...
async function teyitSıralaması(guild) {
    ...
    if(!kanal) return;
    let Data = await Users.find()
    Data = Data.filter(x => guild.members.cache.has(x._id) && x.Records).sort((a, b) => Number(b.Records.length) - Number(a.Records.length)).map((result, index) => {
