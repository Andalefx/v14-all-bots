...
const { 
    ...
    EmbedBuilder // MessageEmbed -> EmbedBuilder (genEmbed'in içinde kullanılıyorsa önemlidir)
} = Discord = require("discord.js"); 

const { genEmbed } = require("../../../../Global/İnit/Embed");

// discord-modals kütüphanesini V14 uyumlu olacak şekilde çağırıyoruz.
// Modal bileşenleri V14'te doğrudan Discord.js'ten geldiği için, bu kütüphanenin 
// sadece event'i yakalamak için kullanıldığını varsayıyoruz.
const { 
    Modal, // V14'te ModalBuilder ile değiştirildi, ancak modalSubmit'ta bu kütüphaneden çekilebilir.
    TextInputComponent, 
    showModal,
    SelectMenuComponent,
...
module.exports = async (modal) => { // Bu dosya modalSubmit event'i olarak çalışıyor.
...
        // İzin Kontrolü: "ADMINISTRATOR" ve "MANAGE_ROLES" stringleri yerine PermissionsBitField.Flags sabitleri
        if(!uye.roles.cache.has(roller.boosterRolü) && (roller.özelRoller && !roller.özelRoller.some(x => uye.roles.cache.has(x))) && !uye.permissions.has(PermissionsBitField.Flags.Administrator) && !uye.permissions.has(PermissionsBitField.Flags.ManageRoles) ) {
            await modal.deferReply({ ephemeral: true })
            return await modal.followUp({content: `Sunucumuza **boost** basmanız gerekmektedir.`})
        }
        if(ayarlar.type && ayarlar.isimyas) {
            if(roller.Yetkiler.some(x => uye.roles.cache.has(x)) && !roller.kurucuRolleri.some(oku => uye.roles.cache.has(oku)) && !uye.permissions.has(PermissionsBitField.Flags.Administrator)) {
...
            // V14 Embed Yapısı: `.addField()` yerine `.addFields()` kullanıldı.
            if(logKanal) logKanal.send({embeds: [new genEmbed().setColor("Orange")
    .setFooter({text: uye.user.tag + " tarafından güncellendi.", iconURL: uye.user.avatarURL({dynamic: true})})
