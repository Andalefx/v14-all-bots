...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
    .setFooter({text: `Bir üyeyi beyaz listeye ekleyip/çıkartmak için ${sistem.botSettings.Prefixs[0]}guard <@acar/ID> komutunu kullanabilirsiniz.`})
...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
              ...
              if(i.customId == "guard_manage") {
                ...
                let guildSettings = await GUILD_SETTINGS.findOne({guildID: sistem.SERVER.ID})
                if(guildSettings) ayarlar = guildSettings.Ayarlar
                let listEmbed = new genEmbed().setColor("WHITE").setDescription(`Aşağı da \`${ayarlar.serverName || message.guild.name }\` sunucusunun beyaz listede ki üyeleri ve rolleri listelenmektedir. Bu listeyi sadece sunucu sahibi güncelleyebilir.
...
**Bilgilendirme**:
Belirtilen üye, rol veya kanal ekli bir beyaz listede bulunuyorsa düğmenin rengi yeşile dönmektedir, bulunmuyor ise düğme füme rengine dönmektedir. :tada:`).setColor("YELLOW")], components: [whiteListRow, whiteListRowTwo, whiteListRowThree, backToMain]}).then(async (msg) => {
            var filter = (i) => i.user.id == message.member.id
