...
// Emojiler için harici bir modül gereksinimi olmadığı için sadece Client ve Message import edildi.

module.exports = {
    Isim: "emojikur",
    Komut: ["emkur"],
    Kullanim: "emkur",
    Aciklama: "Sunucuda eksik olan gerekli emojileri kurar.",
    Kategori: "-",
    Extend: true,
    ...
    onRequest: async function (client, message, args) {
        ...
        const emojis = [
            ...
            // --- ANDALE DEĞİŞİKLİĞİ BURADA YAPILDI ---
            { name: "andale", url: "https://cdn.discordapp.com/emojis/841032678266241054.gif"}, 
            { name: "andale_pat", url: "https://static.wikia.nocookie.net/owobot/images/5/56/SlotsEggplant.png/revision/latest/scale-to-width-down/18?cb=20201231080844"},
            { name: "andale_heart", url: "https://static.wikia.nocookie.net/owobot/images/b/bb/SlotsHeart.png/revision/latest/scale-to-width-down/18?cb=20201231081114"},
            { name: "andale_vis", url: "https://static.wikia.nocookie.net/owobot/images/8/80/SlotsCherry.png/revision/latest/scale-to-width-down/20?cb=20201231081252"},
            { name: "andale_slot", url: "https://static.wikia.nocookie.net/owobot/images/f/f3/SlotsRolling.gif/revision/latest/scale-to-width-down/18?cb=20201231075644"},
            { name: "andale_cf", url: "https://cdn.discordapp.com/emojis/932325559051096084.gif?size=44&quality=lossless"},
            { name: "andale_cfwin", url: "https://cdn.discordapp.com/emojis/932326782223417455.png"},
            { name: "andale_cflose", url: "https://cdn.discordapp.com/emojis/932328417196671026.webp?size=44&quality=lossless"},
        ...
        // Tüm emojileri birleştir
        const allEmojis = [...emojis, ...numEmojis];

        // --- EMOJİ KURMA İŞLEMİ (V14) ---
        
        let createdCount = 0;
        
        for (const x of allEmojis) {
            // Sunucuda emoji zaten varsa atla
            if (message.guild.emojis.cache.find((e) => x.name === e.name)) continue;

            try {
                // V14'te create metodu: create(options)
                ...
                // Emoji oluşturma hatası (örn: slot yok, dosya boyutu büyük)
                console.error(`Emoji (${x.name}) oluşturulurken hata: ${error.message}`);
                // Hata mesajı gönderilebilir, ancak orijinal kodda yoktu.
            }
        }
        
        if (createdCount === 0) {
            message.channel.send("Tüm gerekli emojiler sunucuda zaten mevcut.").catch(e => {});
        } else {
            message.channel.send(`Emoji kurulum işlemi tamamlandı. **${createdCount}** adet emoji oluşturuldu.`).catch(e => {});
        }
    }
};