const { 
    ...
    ButtonBuilder, // v14 ButtonBuilder (Kullanılmasa da import edildi)
    StringSelectMenuBuilder // v14 SelectMenuBuilder'ın yeni ismi: StringSelectMenuBuilder
} = require("discord.js");

const Users = require('../../../../Global/Databases/Client.Users')
const Crews = require('../../../../Global/Databases//_index.schema');
...
                }
                await TalentPerms.updateOne({guildID: message.guild.id}, { $push: {"talentPerms": komutEkle}}, {upsert: true})
                // Üye filtreleme ve rol verme işlemi v14'te de aynıdır.
...
            ekipListesi.push(...acarrrrsikbeni)
            // Filtreleme işlemleri v14'te de aynıdır.
...
    if(!data.length >= 1) return message.channel.send({embeds: [embed.setFooter(`eklemek için ${sistem.botSettings.Prefixs[0]}crew ekle <isimtag> <etiketag> <sorumluluk rolü>`).setDescription(`${message.guild.emojiGöster(emojiler.sarıYıldız)} \`${ayarlar.serverName}\` sunucusunda ekip bulunamadı.`)]});
    ...
    // v14: ActionRowBuilder ve SelectMenuBuilder (StringSelectMenuBuilder) kullanımı
    let satıriki = new ActionRowBuilder().addComponents(
...
        )
    )
    let satır = new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
...
    )
    ...
    // Mesaj gönderme ve kolektör oluşturma v14'te de aynıdır.
...
            if(!crewFind) return;
            ...
            // crewFind.rol kontrolü ters görünüyor (rol varsa değilse rol kuruyor). Orijinal mantık korundu.
