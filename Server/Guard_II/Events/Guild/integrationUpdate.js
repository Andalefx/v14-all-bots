...
    let embed = new genEmbed().setTitle("Sunucuda Entegrasyon Düzenlendi!")
    ...
    // V14 GÜNCELLEMESİ: AuditLogEvent.IntegrationUpdate (52) kullanıldı.
    let entry = await guild.fetchAuditLogs({type: AuditLogEvent.IntegrationUpdate, limit: 1}).then(audit => audit.entries.first());
...
    embed.setDescription(`${entry.executor} (\`${entry.executor.id}\`) tarafından **entegrasyon güncellendi** ve güncellendiği gibi **cezalandırıldı.**`);
