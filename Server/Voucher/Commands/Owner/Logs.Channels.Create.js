const { 
    ...
    MessageSelectMenu, 
    ChannelType, // v14 ChannelType sabitleri
    PermissionFlagsBits // v14 İzin sabitleri
} = require("discord.js");
...
                .setStyle(ButtonStyle.Success), // v14 ButtonStyle
                new ButtonBuilder()
...
    // v14: type: ChannelType.GuildCategory ve PermissionFlagsBits kullanımı
    const log = await message.guild.channels.create({
        name: "A-LOGS",
        type: ChannelType.GuildCategory, // "GUILD_CATEGORY" yerine ChannelType.GuildCategory
        permissionOverwrites: [{
            id: message.guild.roles.everyone.id,
            deny: [PermissionFlagsBits.ViewChannel] // 'VIEW_CHANNEL' yerine PermissionFlagsBits.ViewChannel
        }]
    });
    // v14: type: ChannelType.GuildCategory ve PermissionFlagsBits kullanımı
...
        }]
    });
    normalLoglar.forEach(x => { // .some yerine .forEach kullanılması daha mantıklıdır
        message.guild.channels.create({ name: x, parent: log.id, type: ChannelType.GuildText }); // Text kanalları için ChannelType.GuildText eklendi
    })
    guvenlikLoglar.forEach(x => { // .some yerine .forEach kullanılması daha mantıklıdır
        message.guild.channels.create({ name: x, parent: secLog.id, type: ChannelType.GuildText }); // Text kanalları için ChannelType.GuildText eklendi
    })
