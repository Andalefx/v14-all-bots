const { Client, Message, ActionRowBuilder, ButtonBuilder, SelectMenuBuilder, EmbedBuilder, ButtonStyle} = require("discord.js");

const GUARD_SETTINGS = require('../../../../Global/Databases/Global.Guard.Settings');
const GUILD_SETTINGS = require('../../../../Global/Databases/Global.Guild.Settings');
const ms = require('ms')
const moment = require('moment')
const { genEmbed } = require("../../../../Global/İnit/Embed"); // genEmbed'in EmbedBuilder'ı kapsadığı varsayılmıştır.

// Gerekli global değişkenlerin varsayılan değerleri (Eğer globalde tanımlı değillerse, botunuzun çalışması için bunları tanımlamanız gerekir.)
let sistem = global.sistem || { SERVER: { ID: "SERVER_ID" }, botSettings: { Prefixs: ["!"] } }; 
let ayarlar = global.ayarlar || { serverName: "Sunucu Adı", SMS: false, WHATSAPP: false, TELEGRAM: false, Numaralar: [], chatİzinliler: [] };
let emojiler = global.emojiler || { Onay: '✅', Iptal: '❌' };
let cevaplar = global.cevaplar || { prefix: "!" };

module.exports = {
    Isim: "güvenlik",
    Komut: ["yarram", "guard-settings","guards","amcıkguard","guard"],
    Kullanim: "",
    Aciklama: "",
    Kategori: "-",
    Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if(guildSettings) ayarlar = guildSettings.Ayarlar // Global ayarları güncelle
    ...
    // Embed Oluşturma
    let anaMenu = new genEmbed()
    .setThumbnail(message.guild.iconURL({dynamic: true}))
    // Template literal'deki parantezleme hataları ve fazladan kapatma düzeltildi.
    .setDescription(`Aşağı da \`${ayarlar.serverName || message.guild.name}\` sunucusunun güvenlik sistemi ve güvenliğe takılmış kayıtlar bulunmaktadır. Ayarları güncellemek istiyorsanız aşağıda ki düğmeleri kullanabilirsiniz.

**Güvenlik Bilgisi**
\` • \` Sunucu Güvenliği: \` ${guardSettings.guildProtection ? "Aktif ✅" : "Devre-Dışı ❌"} \`
${guardSettings.guildProtection ? `
\` • \` Limit Sistemi: \` ${guardSettings.limit ? "Aktif ✅" : "Devre-Dışı ❌"} \`
${guardSettings.limit ? `\` • \` Limit Bilgisi: \` ${işlem} ${guardSettings.auditLimit} İşlem Hakkı Bulunmakta! \`` : ""}
` : ""}
...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
    .setFooter({text: `Bir üyeyi beyaz listeye ekleyip/çıkartmak için ${sistem.botSettings.Prefixs[0]}guard <@yuki/ID> komutunu kullanabilirsiniz.`})
        ...
        message.channel.send({embeds: [anaMenu], components: [oneRows]}).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "guard_manager") {
                ...
                let acarRow = new ActionRowBuilder().addComponents([
                    new ButtonBuilder()
                ...
                msg.edit({embeds: [anaMenu], components: [acarRow, backMenu]}).then(async (gmsg) => {
                    var filter = (i) => i.user.id == message.member.id
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        if(i.customId == "guardState") {
                            ...
                            if(guardSettings.guildProtection) {
                                ...
                                acarRow.components[0].setStyle(ButtonStyle.Danger).setLabel(`Güvenliği Aç`)
                               ...
                               if(ayarlar.SMS) acarRow.components[4].setDisabled(false)
                                acarRow.components[0].setStyle(ButtonStyle.Secondary).setLabel(`Güvenliği Kapat`)
                        ...
                        if(i.customId == "limitState") {
                            ...
                            if(guardSettings.limit) {
                                acarRow.components[2].setDisabled(true)
                                acarRow.components[1].setStyle(ButtonStyle.Danger).setLabel(`Limiti Aç`)
                                ...
                                acarRow.components[1].setStyle(ButtonStyle.Secondary).setLabel(`Limiti Kapat`)
                        ...
                        if(i.customId == "limitSettings") {
                            ...
                            message.channel.send({embeds: [new genEmbed().setColor(RANDOM)
.setDescription(`Aşağı da \`${ayarlar.serverName || message.guild.name}\` sunucusunun limit ayarları belirtilmektedir.
...
Olarak ayarlanmış. Ayarı güncellemek için dakika cinsinde işlem süresi belirtin.`).setFooter({text: `Ana menüye dönmek için "iptal" yazabilirsiniz.`})].then(async (ymsg) => {
    ...
    collector.on('collect', async (m) => {
        ...
        if(!sure) return m.reply({content: `${cevaplar.prefix} Lütfen rakam girilmeli veyada boş bırakılmamalıdır.`}).then(x => 
            setTimeout(() => {
                x.delete().catch(err => {})
            }, 7500));
...
Olarak ayarlanmış fakat işlem sayısı ayarlanmamış. Ayarlamak için lütfen kaç işlem olacağını belirtin.`).setFooter({text: `Ana menüye dönmek için "iptal" yazabilirsiniz.`})]})}).then(async (yarramm) => {
    ...
    collector.on('collect', async (m) => {
        ...
        if(!islemcik) return m.reply({content: `${cevaplar.prefix} Lütfen rakam girilmeli veyada boş bırakılmamalıdır.`}).then(x => 
            setTimeout(() => {
                x.delete().catch(err => {})
            }, 7500));
    ...
    collector.on('collect', async (i) => {
        ...
        i.deferUpdate().catch(err => {})
                        ...
                        if(i.customId == "smsState") {
                            ...
                            if(guildSettings) ayarlar = guildSettings.Ayarlar
                            if(ayarlar.SMS) {
                                acarRow.components[3].setStyle(ButtonStyle.Danger).setLabel(`SMS/MMS Aç`)
                                acarRow.components[4].setDisabled(true).setStyle(ButtonStyle.Success).setLabel(`SMS/MMS Gerekli`)
                                ...
                                acarRow.components[4].setDisabled(false).setLabel(`WhatsApp/Telegram Aç`).setStyle(ButtonStyle.Danger)
                                acarRow.components[3].setStyle(ButtonStyle.Secondary).setLabel(`SMS/MMS Kapat`)
                        ...
                        if(i.customId == "wpState") {
                            ...
                            if(ayarlar.WHATSAPP | ayarlar.TELEGRAM) {
                                acarRow.components[4].setStyle(ButtonStyle.Danger).setLabel(`WhatsApp/Telegram Aç`)
                                await GUILD_SETTINGS.updateOne({guildID: sistem.SERVER.ID}, { $set: {"Ayarlar.WHATSAPP": false, "Ayarlar.TELEGRAM": false }}, {upsert: true})
                                
                            } else if(!ayarlar.WHATSAPP || !ayarlar.TELEGRAM) {
                                acarRow.components[4].setStyle(ButtonStyle.Secondary).setLabel(`WhatsApp/Telegram Kapat`)
                        ...
                        let guildSettings = await GUILD_SETTINGS.findOne({guildID: sistem.SERVER.ID})
                        if(guildSettings) ayarlar = guildSettings.Ayarlar
                        işlem = `${moment.duration(ms(guardSettings.auditInLimitTime)).format('h [saatde,] m [Dakikada]')}` // Güncellenmiş işlem bilgisini tekrar al
                        // Template literal'deki parantezleme hataları ve fazladan kapatma düzeltildi.
                        anaMenu.setThumbnail(message.guild.iconURL({dynamic: true})).setDescription(`Aşağı da \`${ayarlar.serverName || message.guild.name}\` sunucusunun güvenlik sistemi ve güvenliğe takılmış kayıtlar bulunmaktadır. Ayarları güncellemek istiyorsanız aşağıda ki düğmeleri kullanabilirsiniz.
...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
              ...
              if(i.customId == "guard_manage") {
                ...
                let guildSettings = await GUILD_SETTINGS.findOne({guildID: sistem.SERVER.ID})
                if(guildSettings) ayarlar = guildSettings.Ayarlar
                let listEmbed = new genEmbed().setDescription(`Aşağı da \`${ayarlar.serverName || message.guild.name }\` sunucusunun beyaz listede ki üyeleri ve rolleri listelenmektedir. Bu listeyi sadece sunucu sahibi güncelleyebilir.

\`\`\`fix
                  Beyaz Liste (Güvenli ID)\`\`\``)
// V14 EmbedBuilder.addFields Kullanımı
.addFields([
    { name: "Limitsiz Full Yönetim", value: `${guardSettings.unManageable.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).length >= 1 ? guardSettings.unManageable.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, inline: false },
    ...
    { name: "Sunucu (Düzenle)", value: `${guardSettings.guildAccess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).length >= 1 ? guardSettings.guildAccess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, inline: false },
    { name: "Bot (Ekle/Çıkar)", value: `${guardSettings.botAccess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).length >= 1 ? guardSettings.botAccess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, inline: false },
    { name: "Roller (Oluştur/Kaldır/Düzenle)", value: `${guardSettings.rolesAcess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).length >= 1 ? guardSettings.rolesAcess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, inline: false },
    { name: "Kanallar (Oluştur/Kaldır/Düzenle)", value: `${guardSettings.channelsAccess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).length >= 1 ? guardSettings.channelsAccess.filter(x => message.guild.members.cache.get(x) || message.guild.channels.cache.get(x) || message.guild.roles.cache.get(x)).map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, inline: false },
    ...
    { name: "Chat Beyaz Liste", value: `${ayarlar.chatİzinliler.length >= 1 ? ayarlar.chatİzinliler.map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || message.guild.channels.cache.get(x)  || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, inline: false }
])
        ...
        let backToMain = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
            .setCustomId("geridongeridon")
...
**Bilgilendirme**:
Belirtilen üye, rol veya kanal ekli bir beyaz listede bulunuyorsa düğmenin rengi yeşile dönmektedir, bulunmuyor ise düğme füme rengine dönmektedir. :tada:`).setColor("#2ECC71")], components: [whiteListRow, whiteListRowTwo, whiteListRowThree, backToMain]}).then(async (msg) => {
            var filter = (i) => i.user.id == message.member.id
            ...
            collector.on('collect', async (i) => {
                // Burada string stiller ButtonStyle sabitlerine dönüştürülmüştü, ancak ikinci kontrol için yeniden kontrol edildi
                ...
                i.deferUpdate().catch(err => {})
                msg.edit({components: [whiteListRow, whiteListRowTwo, whiteListRowThree, backToMain]}).catch(err => {})
            })
        })
    }
