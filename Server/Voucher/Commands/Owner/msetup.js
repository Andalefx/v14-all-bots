// V14 UYUMLULUK: MessageButton -> ButtonBuilder, MessageActionRow -> ActionRowBuilder, MessageSelectMenu -> StringSelectMenuBuilder
// Ayrıca EmbedBuilder, MessageEmbed yerine kullanılmalıdır.
const { Client, Message, ButtonBuilder, ActionRowBuilder, StringSelectMenuBuilder, EmbedBuilder, Formatters, splitMessage, Util } = require("discord.js");
const GUILD_SETTINGS = require('../../../../Global/Databases/Global.Guild.Settings');
// NOT: discord-modals (dcmodal) V13 uyumludur. V14 için Discord.js'in kendi Modal sınıflarını kullanmalısınız.
const { 
  Modal,
  TextInputComponent, 
  showModal
} = dcmodal = require('discord-modals')
let değişkenler = require('../../../../Global/Settings/setup');

let özellikler = değişkenler.değişkenler || []
const bilgilendirme = [
  "Ayarlamak istediğiniz özelliği **.setup <özellik adı> <değer>** şeklinde kullanabilirsiniz.",
  "Tüm ayar listesini görüntülemek için **Ayar Listesi** butonunu kullanabilirsiniz.",
  "Ayarlarınızı değiştirmeden önce yedek almanızı şiddetle tavsiye ederiz.",
  "Bot, komutları çalıştırmak için gerekli tüm izinlere sahip olduğundan emin olun."
];
const buttonSatir = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
        .setCustomId("ayarciklar")
        .setPlaceholder('Kurulum Yönetimi...')
        .addOptions([
            { label: 'Yapılan Ayarları Görüntüle', description: 'Sunucuda tanımlanmış tüm ayarları listeler.', value: 'ayarlar', emoji: '⚙️' },
            { label: 'Güvenlik & Koruma Ayarları', description: 'Tüm güvenlik ve koruma ayarlarına erişim sağlar.', value: 'ayarlar2', emoji: '🛡️' },
            { label: 'Emoji Kurulumu', description: 'Sunucuya özel emojileri kurar.', value: 'emojikur', emoji: '✨' },
            { label: 'Log Kanalı Kurulumu', description: 'Önemli olaylar için log kanallarını kurar.', value: 'logkur', emoji: '📝' },
            { label: 'Seçenek Rol Sistemi Kurulumu', description: 'Kullanıcıların kendi rollerini seçebildiği sistemi kurar.', value: 'seçenekkur', emoji: '✅' },
            { label: 'Özel Komut Yönetimi', description: 'Özel komutları yönetmek için kullanılır.', value: 'özelkomut', emoji: '💡' },
            { label: 'Yasaklı Tag Sistemi', description: 'Yasaklı tag sistemi ayarını yapar.', value: 'yasaklıtag', emoji: '🚫' },
            { label: 'Ekip Tag Sistemi', description: 'Özel taglı ekip rol ve sistemi ayarlar.', value: 'ekipcik', emoji: '🤝' },
            { label: 'Botları Güncelle', description: 'Botların durumunu kontrol edip günceller.', value: 'botsettings', emoji: '🤖' },
            { label: 'PM2 Restle (Tüm Botları)', description: 'Tüm botları PM2 üzerinden yeniden başlatır.', value: 'restleaq', emoji: '🔄' },
            { label: 'Stat Sıfırlama', description: 'Sunucu tüm istatistik verilerini temizler.', value: 'stattemizle', emoji: '🗑️' },
            { label: 'Lisans/Paket Süresi', description: 'Botun süresi ve paket bilgilerini gösterir.', value: 'sure', emoji: '💰' },
            { label: 'Web Panel Girişi', description: 'Websitesi üzerinden yönetim paneline erişim sağlar.', value: 'webpanel', emoji: '🌐' },
            { label: 'Tüm Ayarları Yedekle (Dosya)', description: 'Anlık tüm ayar verisini dosya olarak indirir.', value: 'ayarlar1', emoji: '📁' },
            { label: 'Son 10 Yedeklemeyi Görüntüle', description: 'Veritabanındaki son 10 yedeklemeyi kodlarıyla listeler.', value: 'ayarlar31', emoji: '📜' },
            { label: 'Senkronize Yedek Yükle', description: 'Yedek kodu ile ayarları sunucuya yükler.', value: 'ayarlar62', emoji: '📤' },
            { label: 'Tüm Ayarları Sıfırla', description: 'Sunucunun tüm ayar verisini veritabanından temizler.', value: 'ayarlar3', emoji: '💣' },
            { label: 'Ayar Listesi', description: 'Ayarlanabilir tüm özellik isimlerini listeler.', value: 'ayarlistesi', emoji: '📌' },
        ])
);
...
const sistem = require("../../../../Global/Settings/System.json")
 ...
 module.exports = {
      Isim: "msetup",
      Komut: ["mserver","minstall","msettings","msunucu-yönet","mbot-yönet","msunucuyönet","mkurulum","mlisans"],
      Kullanim: "",
      Aciklama: "",
      Kategori: "-",
      Extend: true,
    ...
    onLoad: function (client) {
      client.on('modalSubmit', async (modal) => {
        if(modal.customId == "senkYükleme") {
          ...
          let backupSettings = require('../../../../Global/Databases/Guild.Settings.Backup');
          ...
          await modal.deferReply({ ephemeral: true })
          return await modal.followUp({content: `${guild.emojiGöster(emojiler.Onay)} Başarıyla **${guild.name} => ${kodbul.Ayarlar.serverName}** sunucu ayarı yüklendi.` , ephemeral: true })
        }
      })
    },
   ...
   onRequest: async function (client, message, args) {
     if(!(ayarlar && ayarlar.staff && ayarlar.staff.includes(message.member.id)) && message.guild.ownerId != message.member.id) return message.reply(cevaplar.noyt).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
      ...
      // Hata Düzeltmesi: Eğer sunucu ayarı veritabanında yoksa, varsayılan bir ayar oluşturuluyor.
      if (!Database) {
          Database = await GUILD_SETTINGS.findOneAndUpdate({guildID: message.guild.id}, {$set: {guildID: message.guild.id, Ayarlar: {}, talentPerms: [], Caches: []}}, {upsert: true, new: true});
      }
      const data = Database.Ayarlar
      let secim = args[0];
      const embed = new genEmbed() 
        if (!secim || !özellikler.some(ozellik => ozellik.name.toLowerCase() == secim.toLowerCase())) {
            return message.channel.send({embeds: [embed.setAuthor(message.member.user.tag, message.member.user.avatarURL({dynamic: true})).setDescription(`**${message.guild.name}** Sunucusunun Yönetim Paneline Hoş Geldiniz.
...
> ${bilgilendirme[Math.floor(Math.random()*bilgilendirme.length)]}`)], components: [buttonSatir]}).then(async (x) => {
                ...
                collector.on('collect', async i => {
                  ...
                  if(i.values[0] === 'webpanel') {
                    if(i.user.id != "1397360217859297321") return await i.reply({content: `${cevaplar.prefix} Sunucu sahibi veya da bot sahibi site üzerinden erişim sağlayabilir..`, ephemeral: true}),
                    message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined),
                    x.delete().catch(err => {})
                    await i.reply({content: `Sizi yönlendiriyorum...`, ephemeral: true})
                  }
                  if(i.values[0] === 'sure') {
                    if(i.user.id != "1397360217859297321") return await i.reply({content: `${cevaplar.prefix} Sistemde profilin girili olmadığı için süre ve ödeme işlemlerine ulaşamazsın.`, ephemeral: true}),
                    message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined),
                    x.delete().catch(err => {})

                    await i.reply({embeds:[new genEmbed().setColor("DARK_AQUA").setDescription(`${message.guild.emojiGöster(emojiler.Icon)} **Merhaba!** Sayın. Selahattin ACAR (\`ceo@acarfx.com\`), ${message.guild.name} (\`ÖMÜR BOYU - LİFE END\`) Sunucusunun ödeme, süre ve paket işlemleri için aşağıda bilgilendirmesi yapılmış ufak kategorilerden kontrol edebilir veya düğmelerden işlem yaparak yada websitesi üzerinden güncelleme gibi işlemleri yapabilirsiniz.`)
                    .addField(`\` ••❯ \` Ödeme Durumu`, `\` • \` **Ödeme Tipi**: Sanal Pos (\`VISA\`)\n\` • \` **Ödeme Tarihi**: 16.12.2021 17:42\n\` • \` **Onay Tarihi**: 16.12.2021 17:43`, true)
                    .addField(`\` ••❯ \` Lisans Bilgisi`, `\` • \` Lisans Tipi: \`Business\`\n\` • \` Lisans Numarası: \`ACAR (Owner)\``, true)], components: [ödeme],ephemeral: true})
...
\` ••❯ \` **${message.guild.name} Sunucusuna Ait Ayarlanabilir Özellikler** (\`${özellikler.length} adet bulunmaktadır.\`): ${özellikler.map(o => `${o.name}`).join(", ")}`, ephemeral: true}), message.react(("1402951436467175455") ? ("1402951436467175455") : undefined), x.delete().catch(err => {})
                  }
                
                  if(i.values[0] === "ayarlar2") {
                    message.react((emojiler.Onay) ? (emojiler.Onay).id : undefined)
                    x.delete().catch(err => {})
                  ...
                  if(i.values[0] === "ayarlar62") {
                    x.delete().catch(err => {})
                    // discord-modals kullanımı olduğu gibi kaldı.
                  ...
                  if(i.values[0] === "ayarlar31") {
                    const backupSettings = require('../../../../Global/Databases/Guild.Settings.Backup');
                    let getir = await backupSettings.find()
...
if(i.customId == "ayarlar_tum") {
    ...
    // Doğrudan Gönderme (Eğer 2000 karakter limitini aşmıyorsa!)
    const tumAyarlarMetni = `
\`\`\`fix
...
${listeTum}`;
    ...
    await i.followUp({
        embeds: [new genEmbed().setColor("Random").setDescription(`${tumAyarlarMetni}`)],
        ephemeral: true // i.reply kullandığınız için followUp'ı ephemeral olarak göndermek daha mantıklı
    });
}
...
${stat}`)], ephemeral: true})
        ...
        if (ozellik.type == "tekil"){
          ...
          if(logKanal) logKanal.send({embeds: [new genEmbed().setColor("Orange").setFooter(message.member.user.tag + " tarafından güncellendi.",message.member.user.avatarURL({dynamic: true}))
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
  ...
  if(args[1] == "-temizle") {
    let logKanal = message.guild.kanalBul("guild-log")
    if(logKanal) logKanal.send({embeds: [new genEmbed().setColor("Orange").setFooter(message.member.user.tag + " tarafından temizlendi.",message.member.user.avatarURL({dynamic: true}))
...
> Güncellenme Tarihi: **\`${tarihsel(Date.now())}\`**`)]})
    ...
    return message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **\`${ozellik.name}\`** isimli ayara \`${tag}\` ayarın eklendi. \`${arr.join(", ")}\` bulunuyor.`)]}).then(x => setTimeout(() => {
      x.delete().catch(err => {})
      message.react((emojiler.Onay) ? (emojiler.Onay).id : undefined).catch(err => {})
    }, 7500));
  }
        }
            ...
            let logKanal = message.guild.kanalBul("guild-log")
            if(logKanal) logKanal.send({embeds: [new genEmbed().setColor("Random").setFooter(message.member.user.tag + " tarafından güncellendi.",message.member.user.avatarURL({dynamic: true}))
